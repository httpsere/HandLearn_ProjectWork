...
(function () {
    ...
    function setBanner(type, text) {
        ...
        els.banner.classList.remove('alert-info','alert-success','alert-warning','alert-error');
    ...
    async function loadModel() {
        ...
        try {
            ...
            setBanner('error',
                'Backend AI non raggiungibile su ' + NODE_URL + '. Avvia "npm start" nella cartella server/ e ricarica la pagina.');
    ...
    async function startCamera() {
        ...
        try {
            camera = new Camera(els.video, {
                onFrame: async () => {
                    if (els.video.readyState === 4) await hands.send({ image: els.video });
                },
            ...
            setBanner('error', 'Impossibile accedere alla webcam. Concedi i permessi e ricarica.');
            els.loader.style.display = 'none';
        }
    }
    ...
    function onResults(results) {
        ...
        } catch (err) { /* model not yet loaded */ }
        ...
        if (isMatch && confidence >= 0.85) {
            ...
            color = '#f59e0b';
            setFeedback('warn', `Stai facendo "${predicted}". Prova invece "${target}".`);
        } else {

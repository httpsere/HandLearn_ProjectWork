...
(function () {
    ...
    let timeLeft = DURATION, score = 0, combo = 0, comboMax = 1, lives = 3;
    let target = null, lastTarget = null, lastSuccessAt = 0;
    ...
    function pickRandomTarget() {
        ...
        let next; do { next = labels[Math.floor(Math.random() * labels.length)]; }
        while (next === lastTarget && labels.length > 1);
        lastTarget = next; target = next;
        elTarget.textContent = next;
        elTargetV.innerHTML = makeHandSvg();
    }
    ...
    function setFeedback(state, text) {
        ...
        elFeedback.classList.add({
            good: 'is-good', warn: 'is-warn', bad: 'is-bad', neutral: 'is-neutral'
        }[state] || 'is-neutral');
    ...
    async function loadAi() {
        try {
            ...
            labels = await fetch(NODE_URL + '/model/labels.json').then(r => r.json());
            return true;
        } catch (err) {
    ...
    function onResults(results) {
        ...
        const features = lm.flatMap(p => [
            (p.x-base.x)/scale, (p.y-base.y)/scale, (p.z-base.z)/scale
        ]);
        ...
        tf.tidy(() => {
            ...
            let best = 0;
            arr.forEach((s,i) => { if (s > arr[best]) best = i; });
            predicted = labels[best]; confidence = arr[best];
        });
        ...
        if (predicted === target && confidence >= 0.85) {
            ...
            color = '#f59e0b';
            setFeedback('warn', `Stai facendo "${predicted}", invece di "${target}".`);
        }

        if (window.HAND_CONNECTIONS) drawConnectors(ctx, lm, HAND_CONNECTIONS, { color, lineWidth: 3 });
    ...
    async function startGame() {
        ...
        timeLeft = DURATION; score = 0; combo = 0; comboMax = 1; lives = 3;
        elScore.textContent = '0'; elCombo.textContent = '1'; elTime.textContent = DURATION + 's';
        updateLives(); setFeedback('neutral', 'Pronto, mostra il segno!');
    ...
    async function endGame() {
        ...
        elEndTitle.textContent = score >= 200 ? 'Incredibile!' : score >= 100 ? 'Ottimo!' : 'Tempo scaduto';
        elEndText.textContent  = score >= 200 ? 'Sei velocissimo. Prova a battere il record.'
                              : score >= 100 ? 'Ottimo punteggio. Riprova per migliorare.'
                              : 'Continua ad allenarti nelle lezioni e nella sezione Esercitati.';

        // salva sulla classifica Node + nel DB se loggato
        const playerName = (document.querySelector('.user-chip')?.textContent.trim()) || 'Player';
        try {
            ...
            elBoard.innerHTML = top.map(s => `<li>${escapeHtml(s.name)} — <strong>${s.score}</strong></li>`).join('');
        } catch (e) {
            elBoard.innerHTML = '<li>Classifica non disponibile (server AI offline).</li>';
        }
        try {
            fetch('../api/save_progress.php', {
                method:'POST', headers:{'Content-Type':'application/json'},
                body: JSON.stringify({ type:'score', gioco:'sfida-ai', punteggio: score })
            }).catch(()=>{});
        } catch(e){}
